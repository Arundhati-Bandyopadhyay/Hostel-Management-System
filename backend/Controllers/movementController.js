...
const MoveOutTime = async (req, res) => {
  try {
    ...
    if (StudentGone && StudentGone.MoveOut_Time) {
      ...
      const time = await Movement.create(Moveouting_Time, {
  ...
  try {
    ...
    if (StudentGone && StudentGone.MoveIn_Time) {
      ...
      res
        .status(400)
        .json(
          "This is student has not yet gone outside,please check entry details!"
        );
    } else {
      const MoveIn_Time = {
        MoveIn_Time: Date.now(),
      };
      const time = await Movement.update(MoveIn_Time, {
        where: { studentId: req.body.id },
      });
